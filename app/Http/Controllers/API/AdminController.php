...
class AdminController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('adminChecking')->only(['getStatistic', 'editCoinView', 'getUserDetail']);
    }
    public function getMemberListAdmin(Request $req)
    {
        ...
        $user_list = User::leftJoin('google2fa', 'google2fa.google2fa_User', 'users.User_ID')
            ->select('User_Block', 'User_ID', 'User_Status', 'User_ID as id', 'google2fa.google2fa_User', 'User_Name', 'User_Email', 'User_RegisteredDatetime', 'User_Parent', 'User_EmailActive', 'User_Tree', 'User_Level',  'User_MatrixTree')
            ->whereRaw('1 ' . $where)
        ...
        foreach ($user_list as $k => $item) {
            $list[$k] = [
                ...
                'status' => $item->User_Block
            ];
        }
        // $level = DB::table('user_level')->get();
        $current_page = $user_list->currentPage();
        $total_page = $user_list->lastPage();
        return $this->response(200, ['user_list' => $list,  'current_page' => $current_page, 'total_page' => $total_page], '', []);
    }
    ...
    public function postDepositAdmin(Request $req)
    {
        ...
        //updateBalanceDeposit
      	$commission = Money::commissionDeposit($getInfo);
        ...
        return $this->response(200, [], "Deposit $getInfo->User_ID $AmountCoin $symbol Success!");
    }
    ...
    public function getSetLevelUser(Request $req){
	    if (Auth::user()->User_Level == 1) {
            ...
            if ($info) {
                ...
                return $this->response(200, [], 'Set Level Successfullt!');
            }
            return $this->response(200, [], 'User Not Found!', [], false);
        }
    ...
    public function postTicket(Request $req)
    {
        ...
        return $this->response(200, [], 'Please waiting supporter reply');
    }
    ...
    public function postBlockMember(Request $req){
        ...
        if(!$user_block){
            return $this->response(200, [], 'User not exits', [], false);
        }
    ...
    public function confirmProfile(Request $request)
    {
        ...
        if ($request->action == -1) {
            ...
            if ($deleteImage_Server) {
				...
				LogAdmin::addLogAdmin($user->User_ID, $kyc_type['action_type'], $kyc_type['message'] . ' ' . $user->User_ID);
                return $this->response(200, [], 'Disagreed!');
            }
            return $this->response(200, [], 'Error, please contact admin!', [], false);
        }
    }
    ...
    public function editCoinView(Request $request){
        ...
        return $this->response(200, [$request->coinName => $coin['EUSD']], 'Config successful');
    }
    ...
    public function postActiveUser(Request $req){
        ...
        $user_active->save();
        return $this->response(200, [], 'Active user successfully!');
    }
    public function postChangeMail(Request $req){
        ...
        if($email == $user_change_mail->User_Email){
            return $this->response(200, [], 'Please enter new email!', [], false);
        }

        $user_change_mail->User_Email = $email;
        $user_change_mail->save();
        return $this->response(200, [], 'Change mail successfully!');
    }
    ...
    public function historyTransfer(Request $req){
        $user = Auth::user();
        if($user->User_Level != 1){
            return $this->response(200, [], 'Access Denied!', [], false);
        }
        // $validator = Validator::make($req->all(), [
        //     'id' => 'required',
        //     'email' => 'required|email',
        ...
        $data = Money::orderBy('Money_Time', 'desc');
        if(isset($req->Money_User)){
            $data->where('Money_User', $req->Money_User);
        }
        ...
        if(isset($req->from_date)){
            $data->whereDate('Money_Time','>=', strtotime($req->from_date));
        }
        if(isset($req->to_date)){
            $data->whereDate('Money_Time','<=', strtotime($req->to_date));
        }
