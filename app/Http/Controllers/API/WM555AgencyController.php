...
class WM555AgencyController extends Controller
{
    //
    public $config;
    ...
    public function __construct()
    {
        ...
        $this->keyAPI = [
            'fringo' => 'giHCGnkgMr3QfOZTfYHrRSotww922o3GDht2i1IMq4QF6IWOMa',
        ];
    }

    function checkKeyAPI($key, $project){
        $keyAPI = $this->keyAPI;
        if(!isset($keyAPI[$project])){
            return false;
        }
        if($keyAPI[$project] != $key){
            return false;
        }
        return true;
    }
...
//        if ($user->User_Level != 1 && $this->is_maintain == 1) return $this->response(200, [], 'The function is maintained!!!', [], false);
//        //dd($user);
//        if ($user->User_WM555 != 1) {
//            return $this->response(200, [], 'You have not registered!', [], false);
//        }
//
//        if ($user->User_WM_Password !== $request->password) {
//            return $this->response(200, [], 'Old password is incorrect!', [], false);
//        }
        if ($request->password === $request->new_password) {
            return $this->response(200, [], trans('notification.New_password_and_old_password_cannot_be_the_same!'), [], false);
        }
        $username = 'now' . $request->username;
        $dataRaw = [
        ...
        if (!$data || $data->error_code != 0 || $data->data->status == false) {
            ...
            return $this->response(200, [], $data->data->message ?? 'Change password fail! Please try again!', [], false);
        }
    ...
    public function registerWM(Request $request)
    {
        ...
        $user = User::find($request->user()->User_ID);
        if($user->User_Level != 1 && $this->is_maintain == 1) return $this->response(200, [], 'The system is maintained!!!', [], false);
        //dd($user);
        if ($user->User_WM555) return $this->response(200, [], 'You Registered!', [], false);
...
//		dd( $this->config['url'].'addmember?apikey='.$apiKey , $data, json_encode($dataRaw));
        if (!$data || $data->error_code != 0 || $data->data->status == false) {
            ...
            return $this->response(200, [], $data->data->message ?? 'Register fail! Please try again!', [], false);
        }
    ...
    public function depositWM(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'amount' => trans('notification.amount_required'),
        ]);
...
//        dd($this->config['url'] . 'deposited?apikey=' . $apiKey, $data, json_encode($dataRaw));
        ...
        if (!$data || $data->error_code != 0 || $data->data->status == false) {
            ...
            return $this->response(200, [], $data->data->message ?? 'Deposit fail! Please try again!', [], false);
        }
...
//        dd($data, $this->config['url'], $this->config['url'].'withdraw?apikey='.$apiKey, json_encode($dataRaw));
        ...
        if (!isset($data) || !$data || $data->error_code != 0 || $data->data->status == false) {
            ...
            return $this->response(200, [], $data->data->message ?? 'Withdraw fail! Please try again!', [], false);
        }
        LogUser::addLogUser($user->User_ID, 'Deposit Success WM555', 'URL: (' . ($this->config['url'] . 'deposited?apikey=' . $apiKey) . ') PARAM: (' . json_encode($dataRaw) . ') RESPONSE: ' . ($res->getBody()->getContents()), $request->ip());
        // Minus balance in 123betnow
        $arrayInsert = array(
