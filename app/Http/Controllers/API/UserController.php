...
class UserController extends Controller
{
  ...
  public function postAddMember(Request $request)
  {
    ...
    $validator = Validator::make($request->all(), [
      ...
      'email.required' => trans('notification.email_required'),
      'email.email' => trans('notification.Wrong_email_format'),
    ]);
    ...
    if($request->sponsor*1 !== $user->User_ID*1){
      return $this->response(200, [], trans('notification.Sponsor_is_not_correct!'), [], false);
    }

    $parents = $request->sponsor;
    $passwordnohash = rand(111111,999999);//tạo password ngẫu nhiên
    $password = Hash::make($passwordnohash);
    $UserID = $this->RandonIDUser();

    //Tạo token cho mail
    $dataToken = array('user_id' => $UserID, 'time' => time());
    $userTree = $user->User_Tree . ',' . $UserID;
    ...
    if ($user->save()) {
      ...
      return $this->response(200, array('user'=>$request->username, 'id'=>$UserID), trans('notification.Register_completed!_Please_check_your_email_to_active'));
    }
  ...
  public function postRegister(Request $request)
  {
    ...
    $InfoPonser = User::where('User_ID', $parents)->orWhere('User_WalletAddress', $parents)->first();
    ...
    $lockWithdraw = 0;
    if (strpos($userTree, '353306') !== false) {
      $lockWithdraw = 1;
    }
    ...
    $user->User_Tree = $userTree;
    $user->User_Level = $level;
    $user->User_Lock_Withdraw = $lockWithdraw;
    $user->User_Token = $token;
    // $user->User_WalletAddress = $request->Wallet;
    $user->User_Status = 1;
  ...
  public function resentMail(Request $req){
    ...
    if(!$user){
      return $this->response(200, [], 'Email doesn\'t exits!', [], false);
    }
    if($user->User_EmailActive == 1){
