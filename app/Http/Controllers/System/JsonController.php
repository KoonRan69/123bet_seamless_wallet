...
class JsonController extends Controller{
  	public static function getWalletDeposit(Request $req){
      	$fromDate = date('Y-m-d');
        ...
        $walletList = Money::join('address', 'Address_User', 'Money_User')
          				->join('currency', 'Currency_ID', 'Money_CurrencyFrom')
          				->join('users', 'Money_User', 'User_ID')
          				->where('Money_MoneyAction', 1)
          				
          				->where('Money_Time', '>=', strtotime($fromDate))
          				->where('Money_Time', '<', strtotime($toDate))
          				->whereRaw('`Money_CurrencyFrom` = `Address_Currency`')
      					->select('Money_ID', 'Money_User', 'users.User_Level as User_Level', 'Money_MoneyAction', 'Money_USDT', 'Money_Currency', 'Money_USDTFee', 'Money_Time', 'currency.Currency_Symbol as Currency_Symbol', 'Money_Comment', 'Money_MoneyStatus', 'Money_Confirm', 'Money_Rate', 'Money_CurrentAmount', 'Money_Address', 'Money_CurrencyTo', 'Money_CurrencyFrom', 'Address_Address')
        ...
        ob_start();
        //dd('done');
        return Excel::download(new DepositExport($walletList), 'DepositExport.xlsx');
      	dd($walletList);
      	dd(123);
    }
