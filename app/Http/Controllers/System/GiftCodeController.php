...
// 		$currency = DB::table('currency')->where('Currency_Active', 1)->get();
		...
		$listGiftCode = DB::table('giftcode')->leftJoin('package_giftcode', 'GiftCode_Package', 'Package_ID');
		if($request->user_created){
			$listGiftCode = $listGiftCode->where('GiftCode_User', $request->user_created);
		} 

		if($request->user_used){
			$listGiftCode = $listGiftCode->where('GiftCode_User_Use', $request->user_used);
		}

		if($request->package_name){
			$listGiftCode = $listGiftCode->where('package_giftcode.Package_Name', $request->package_name);
		}

		// return $listGiftCode->orderByDesc('GiftCode_ID')->get();

		if($request->export == 1){
			ob_end_clean();
            ob_start();
            return Excel::download(new GiftCodeExport($listGiftCode->orderByDesc('GiftCode_ID')->get()), 'GiftCodeExport.xlsx');
		}

		$listGiftCode = $listGiftCode->orderBy('GiftCode_ID', 'DESC')->paginate(50);
// 		dd($listGiftCode);
