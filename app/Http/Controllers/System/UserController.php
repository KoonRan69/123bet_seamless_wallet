...
*/
        if($req->otp != $user->User_OTP){
	        return redirect()->back()->with(['flash_level'=>'error', 'flash_message'=>'OTP is wrong!!!']);
        }
 	    $address = $req->address;
...
*/
    ...
    public function PostKYC(Request $request)
    {
        ...
        if ($passportImageStatus and $passportImageSelfieStatus) {
            $insertProfileData = [
                ...
                // 'Profile_Full_Name' => $passportFullName,
    ...
    public function postNamePhone(Request $request){
        ...
        if($checkExist && $user_update->User_Phone && $user_update->User_FullNameUser){
            return redirect()->back()->with(['flash_level'=>'error', 'flash_message'=>'You Not Change Your Profile After Confirm KYC']);
        }
        ...
        if($user_update){
          ...
          $user_update->save();
          return redirect()->back()->with(['flash_level' => 'success', 'flash_message' => "Update profile success"]);
        }
