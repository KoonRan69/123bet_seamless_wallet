...
// 		dd($user);
	...
	public function getSetNew(Request $req, $id){
        $check_noti_image = DB::table('post_notifications')->where('id', $id)->first();
		if($check_noti_image->is_new == 1){
			$updateNoti_image = DB::table('post_notifications')->where('id', $id)->update([
				'is_new'=> 0
			]);
			return redirect()->back()->with(['flash_level' => 'success', 'flash_message' => 'Unset new notification Success!']);
		}else{
			
			$updateNoti_image = DB::table('post_notifications')->where('id', $id)->update([
				'is_new'=> 1
			]);
			return redirect()->back()->with(['flash_level' => 'success', 'flash_message' => 'Set new notification Success!']);
		}
