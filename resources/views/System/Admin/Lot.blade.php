        ...
        <div class="row">
                        ...
                        <div class="nav-active-border left b-primary">
                            <ul class="nav nav-sm flex-column">
                                <li class="nav-item active">
                                    <a class="nav-link active block text-bold f-20 m-b" style="color:#777!important;"  href data-toggle="tab" data-target="#tab-sales">SET SALES</a>
                                </li>
                                @for($i = 1;$i <= count($percent); $i++)
                                <li class="nav-item">
                                    <a class="nav-link block text-bold f-20 m-b" style="color:#777!important;"  href data-toggle="tab" data-target="#tab-{{$i}}">${{$arrMinMax[$i]['min']}} - ${{$arrMinMax[$i]['max']}}</a>
                                </li>
                                @endfor
                            </ul>
                        </div>
                ...
                <div class="col-sm-9 col-lg-10">
                    <div class="panel panel-default card-view tab-content pos-rlt">
                        <div class="tab-pane active m-t" id="tab-sales">
                                    ...
                                    <table class="table table-hover b-t m-t">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="width: 20%">Edit Sales</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach($lotSales as $upt1)
                                            <tr>
                                                <td>{{$upt1->lot_Date}}</td>
                                                <td>$ {{number_format($upt1->lot_Sales,4)}}</td>
                                                <td width="10%">
                                                <form action="{{ route('system.admin.postChangeLotSales') }}" method="post">@csrf
                                                    <input type="number" step="any" name="sales" class="form-control" value="{{$upt1->lot_Sales*1}}">
                                                    <input type="hidden" name="ID" value="{{$upt1->lot_ID}}"><button class="btn btn-rounded btn-primary ">Edit</button>
                                                </form>
                                                </td>
                                            </tr>
                                            @endforeach
                        ...
                        @for($i = 1;$i <= count($percent); $i++)
                        <div class="tab-pane m-t" id="tab-{{$i}}">
                            <div class="box hover-box-shadow">
                                <div class="table-responsive">
                                    <table class="table table-hover b-t m-t">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="width: 20%">Edit Member</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{$upt1->lot_Date}}</td>
                                                <td>{{$upt1->lot_Member*1}}</td>
                                                <td width="10%">
                                                <form action="{{ route('system.admin.postChangeLotMember') }}" method="post">@csrf
                                                    <input type="number" step="any" name="member" class="form-control" value="{{$upt1->lot_Member*1}}">
                                                    <input type="hidden" name="ID" value="{{$upt1->lot_ID}}"><button class="btn btn-rounded btn-primary ">Edit</button>
                                                </form>
                                                </td>
                                            </tr>
