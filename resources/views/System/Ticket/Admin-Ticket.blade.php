                                                    ...
                                                    <table id="myTable1"
                                                        ...
                                                        <tbody>
                                                            @foreach ($ticket as $t)
                                                                @php
                                                                    $hide = 1;
                                                                    $checkHideStatus = DB::table('ticket')
                                                                    ...
                                                                    if ($checkHideStatus) {
                                                                        ...
                                                                        $findlastRep = DB::table('ticket')
                                                                            ->where('ticket_ReplyID', $t->ticket_ID)
                                                                            ->orderBy('ticket_ID', 'DESC')
                                                                            ->first();
                                                                        $messNum = 0;
                                                                        $getComment = [];
                                                                        if (!$findlastRep) {
                                                                            $getComment = DB::table('ticket')
                                                                                ->Where('ticket_ID', $t->ticket_ID)
                                                                            ...
                                                                            $getInfo = App\Model\User::whereIn('User_Level', [1,2,3])
                                                                                ->where('User_ID', $findlastRep->ticket_User)
                                                                                ->first();
                                                                            ...
                                                                            if ($getInfo) {
                                                                                ...
                                                                                $keyItem = 1;
                                                                                $getListReplyed = DB::table('ticket')
                                                                                    ->Where('ticket_ReplyID', $t->ticket_ID)
                                                                                    ->orderBy('ticket_ID', 'DESC')
                                                                                    ->get();
                                                                                foreach ($getListReplyed as $item) {
                                                                                    $findUserAdmin = App\Model\User::whereIn('User_Level', [1,2,3])
                                                                                        ->where('User_ID', $item->ticket_User)
                                                                                        ->first();
                                                                                    if (!$findUserAdmin) {
                                                                                        //don't User Admin
                                                                                        $messNum = $keyItem;
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <a href="{{ route('getTicketDetail', $t->ticket_ID) }}"
                                                                            class="btn btn-primary btn-rounded">Details
                                                                            <span
